...
class InvalidAlternativesException(Exception):
    pass
...
def alts(*ps: Tuple[Parser[T], ...]) -> Parser[List[T]] :
    """
    Parses alternatives.  This is a generalization of `alt` and can
    handle an arbitrary number of alternatives.
    """
    if len(ps) < 2:
        raise InvalidAlternativesException("The alts parser must have at least two parsers.")

    result = ps[-1]  # initial parser is the last parser
    i = len(ps) - 2  # index starts from the second-last element

    while i >= 0:
        result = alt(ps[i], result)  # accumulate alternatives
        i -= 1                       # next element
...
def psepby1(p: Parser[T], sep: Parser[str]) -> Parser[List[T]] :
    """
    Runs `p` as many times as possible, running `sep` in between.
    ...
    :type sep: Parser[str]
    """
    # first run p parser, and then run suffix parser as many times as possible
    return pseq(p, pmany0(pright(sep, p))) > (lambda tup: [tup[0]] + tup[1])
...
def pad0(p: Parser[T]) -> Parser[T] :
    """
    Declares a parser that accepts zero or more whitespace characters before
    or after the given parser.
    :param p: A parser
    :type p: Parser[T]
    """
    return pbetween(pws0(), p, pws0())

def pad1(p: Parser[T]) -> Parser[T] :
    """
    Declares a parser that accepts at least one whitespace characters before
    and after the given parser.
    :param p: A parser
    :type p: Parser[T]
    """
    return pbetween(pws0(), p, pws0())
...
# i6 = Input("a")
# i7 = Input("")
# print(psepby1(pitem(), pad0(pchar(',')))(i5))
# print(psepby1(pitem(), pad0(pchar(+',')))(i6))
# print(psepby1(pitem(), pad0(pchar(',')))(i7))
# i8 = Input("ab12zz!")
# print(pmany1(alts(pdigit(), pletter()))(i8))